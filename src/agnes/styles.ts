...
export const AGNES_BASE_PROMPT =
  '把参考图重新设计成适合拼豆图纸生成的低分辨率拼豆分色稿，而不是写实照片滤镜。重点突出主体，放大主体，让主体占画面面积的 88-96%。使用纯色平涂色块，不要渐变、不要柔和阴影、不要抗锯齿、不要真实照片纹理。保持高对比、硬边缘、连续粗深色轮廓、有限配色和适合 24-56 格小尺寸拼豆网格的图标化构图。必须按 36×36 也能读懂来设计：所有关键线条和五官都要能落到实际拼豆格上，眼睛、嘴巴、脸型、头发边缘和服装大特征必须完整、清楚、不可断裂；眼睛必须是左右分明的独立深色块，不能让刘海、眉毛、眼镜框或外轮廓代替眼睛。背景必须是透明或单一浅色空白，并且不应作为拼豆内容；不要大面积空白边距，不要让小主体漂浮在大片空背景中。'
