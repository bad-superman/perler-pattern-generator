...
function buildGridSizePrompt(gridSize?: number) {
  ...
  if (gridSize <= 40) {
    return '最终图纸目标约 36×36 格，请按 36 格小头像/小徽章来重新设计，而不是把大图直接缩小。主体必须接近铺满方形画布，脸部和表情优先级最高：头部约占 22-27 格高，眼睛必须做成 3×3 或更大的深色可读形状，两只眼要左右分明；嘴巴必须是 3-5 格的完整深色短弧/短线，不能只剩 1 格点；外轮廓至少 2 格厚或有连续阶梯线，禁止断边、断嘴、断眼。最多 6-7 个主体主色，使用艳丽高饱和大色块；删除所有小于 2 格的装饰、发丝、布料纹理、背景和碎阴影，禁止在边角生成远离主体的星星、爱心、小贴纸、文字或漂浮装饰，以免主体在 36 格图纸中被缩小。'
  }
  if (gridSize <= 48) {
